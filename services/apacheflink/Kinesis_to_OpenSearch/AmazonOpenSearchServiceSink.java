...
public class AmazonOpenSearchServiceSink {
    ...
    private static <T> IndexRequest createIndexRequest(T element, String index) {
        try {
            ...
            throw new RuntimeException("Failed to serialize element to JSON", e);
        }
    }
    ...
    public static <IN> OpensearchSink<IN> buildAmazonOpenSearchSink(
            String index, String endpointUrl, RestClientFactory clientFactory) {
        ...
        return new OpensearchSinkBuilder<IN>()
                ...
                .setEmitter((element, ctx, indexer) -> indexer.add(createIndexRequest(element, index))) // element.toString()はNG
                .build();
    }
